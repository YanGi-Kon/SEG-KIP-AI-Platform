...
.a4-preview .act-signers-value{min-height:22px;padding:0 4px 1px;border-bottom:1px solid #111;text-align:center;display:flex;align-items:flex-end;justify-content:center;word-break:break-word;color:transparent;text-shadow:none;position:relative}
.a4-preview .act-signers-value.has-signature{min-height:62px}
.a4-preview .act-signature-box{position:absolute;left:50%;bottom:-1px;transform:translateX(-50%);width:140px;height:58px;display:flex;align-items:flex-end;justify-content:center;overflow:hidden;box-sizing:border-box;pointer-events:none}
      ...
      if (approvalContext.workspaceId && isDatabaseConfigured()) {
        try {
          ...
          finalPdfExport = {
            ...
            error: processed?.status === 'failed_permanent' ? processed.lastError : undefined,
            errorCode: processed?.status === 'failed_permanent' ? processed.lastErrorCode : undefined,
          };
        } catch (queueError) {
          finalPdfExport = {
