...
export function renderToPeriodA4(bundle, { workspaceName = '', approvals = [] } = {}) {
  ...
  const approvalMap = new Map(approvals.map((row) => [clean(row.signerId), row]));
  const bodyRows = [];
  let sequence = 0;
  ...
  const approvalRows = approvals.length
    ? `<div class="to-a4-approvals"><div class="to-a4-approval-title">Имзо чекувчилар ҳолати</div>${approvals.map((row) => `<div class="to-a4-approval-row"><span>${esc(row.position || '')}</span><b>${esc(row.fio || '')}</b><span>${esc(row.status || 'Кутилмоқда')}</span></div>`).join('')}</div>`
    : '';
...
export async function sendToPeriodForApproval(workspace, year, month, req) {
  ...
  if (!targets.length) {
    const error = new Error('Bu Workspace uchun faol email manzilli imzo chekuvchi topilmadi');
  ...
  for (const signer of targets) {
    ...
    const approval = await upsertApproval(workspace, {
      id: approvalId,
      docKey,
      signerId: signer.id,
    ...
    const text = [`TO hujjatini tasdiqlash talab qilinadi.`, '', `Obyekt: ${clean(workspace.name)}`, `Davr: ${label}`, `Imzolovchi: ${clean(signer.fullName)}`, '', 'Hujjatni A4 ko‘rinishda ochish va tasdiqlash:', link].join('\n');
    ...
    try {
      ...
      results.push({ signer: signer.fullName, email: signer.email, status: 'email-failed', code: emailError.code || 'EMAIL_HTTP_FAILED', error: emailError.message });
    }
  }
  return {
    key: docKey,
...
async function approvalContext(token, req, { markOpened = false } = {}) {
  ...
  if (!approval) throw new Error('TO tasdiqlash yozuvi topilmadi');
  if (approval.tokenHash && approval.tokenHash !== sha256(token) && approval.status !== 'Тасдиқланди') {
    throw new Error('TO tasdiqlash havolasi eskirgan');
  }
  let next = approval;
  ...
  return { payload, workspace, bundle, approval: next };
}
