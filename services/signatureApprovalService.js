...
export async function getApprovalStatus(token) {
  const { config } = await approvalConfig(token);
  const { approval } = await findApprovalByToken(config, token);
  return {
    approvalId: clean(approval.id),
    actNo: clean(approval.actNo),
    signerId: clean(approval.signerId),
    fio: clean(approval.fio),
    gmail: clean(approval.gmail),
    status: clean(approval.status),
    approvedAt: clean(approval.approvedAt),
    approved: clean(approval.status) === 'Тасдиқланди',
  };
}
...
export function renderApprovalPage({ approval, document, csrfToken }, token) {
  ...
  const expected = {
    approvalId: clean(approval.id),
    signerId: clean(approval.signerId),
    gmail: clean(approval.gmail).toLowerCase(),
  };
  return `<!doctype html><html lang="uz"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(document.actNo)} — Tasdiqlash</title><style>body{margin:0;background:#071427;color:#eaf7ff;font-family:Arial,sans-serif}.wrap{max-width:980px;margin:0 auto;padding:20px}.head{display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap}.badge{padding:8px 12px;border-radius:999px;background:${isApproved ? '#166534' : '#92400e'}}.paper{background:#fff;color:#111;padding:24px;margin:18px 0;border-radius:12px;overflow:auto}.a4-preview{max-width:210mm;min-height:297mm;margin:auto;background:#fff;color:#111;padding:18mm;box-sizing:border-box;font-family:'Times New Roman',serif}.actions{display:flex;justify-content:center;margin:20px}.approve{border:0;border-radius:10px;padding:14px 24px;font-size:17px;font-weight:800;background:#22c55e;color:#052e16;cursor:pointer}.approve:disabled{opacity:.55}.msg{text-align:center;min-height:24px}.approver{text-align:center;color:#cbd5e1;font-size:13px;margin:-8px 0 10px}.approver b{color:#fff}</style></head><body><div class="wrap"><div class="head"><div><h1>Hujjatni tasdiqlash</h1><p>${escapeHtml(approval.position)} — ${escapeHtml(approval.fio)}</p></div><div class="badge" id="status">${escapeHtml(approval.status)}</div></div><div class="paper">${safeHtml}</div><div class="approver">Ushbu havola: <b>${escapeHtml(approval.fio)}</b> · ${escapeHtml(approval.gmail)}</div><div class="actions"><button class="approve" id="approveBtn" ${isApproved ? 'disabled' : ''}>${isApproved ? 'Tasdiqlangan' : 'Tasdiqlash'}</button></div><div class="msg" id="msg"></div></div><script>const token=${JSON.stringify(token)};const csrfToken=${JSON.stringify(csrfToken)};const expectedApproval=${JSON.stringify(expected)};const b=document.getElementById('approveBtn');const statusEl=document.getElementById('status');const msg=document.getElementById('msg');function sameApproval(d){return String(d?.approvalId||'')===expectedApproval.approvalId&&String(d?.signerId||'')===expectedApproval.signerId&&String(d?.gmail||'').toLowerCase()===expectedApproval.gmail;}function paint(d){if(!sameApproval(d)){b.disabled=true;statusEl.textContent='Havola mos emas';msg.textContent='Tasdiqlovchi havolasi boshqa yozuv bilan mos kelmadi.';return;}const approved=Boolean(d.approved);statusEl.textContent=approved?'Тасдиқланди':'Кутилмоқда';b.textContent=approved?'Tasdiqlangan':'Tasdiqlash';b.disabled=approved;msg.textContent=approved&&d.approvedAt?'Tasdiqlangan vaqt: '+d.approvedAt:'';}async function refreshIndividualStatus(){try{const r=await fetch('/api/document/approve/status/'+encodeURIComponent(token),{cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Status xatosi');paint(d);}catch(e){msg.textContent=e.message;}}b.addEventListener('click',async()=>{b.disabled=true;msg.textContent='Tasdiqlanmoqda...';try{const r=await fetch('/api/document/approve',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,csrfToken})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Xato');const individual={approvalId:d.approval?.id,signerId:d.approval?.signerId,gmail:d.approval?.gmail,status:d.approval?.status,approvedAt:d.approval?.approvedAt,approved:d.approval?.status==='Тасдиқланди'};if(!sameApproval(individual))throw new Error('Tasdiqlash javobi ushbu tasdiqlovchiga tegishli emas');paint(individual);msg.textContent='Hujjat muvaffaqiyatli tasdiqlandi.';}catch(e){b.disabled=false;msg.textContent=e.message;}});window.addEventListener('pageshow',()=>void refreshIndividualStatus());void refreshIndividualStatus();</script></body></html>`;
}
